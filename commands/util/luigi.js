  ...
  Type = "10+1";
   ...
   let ObjectToUpdateString = GetObjectToUpdateString(ObjectToUpdate, AmountToRemove);
 		...
 		CurrentAmount = CurrentAmount - AmountToRemove; // Remove the spent amount
          ...
          let CurrentTimesUsedSkill = fs.readFileSync("./PetBot/settings/skills/ChikaSkill.txt");
      if(CurrentTimesUsedSkill == 0){
        return;
      }
      fs.writeFileSync("./PetBot/settings/skills/ChikaSkill.txt",parseInt(CurrentTimesUsedSkill) - 1);
...
function getRarityNormal(RandomScoutRarity){
  ...
  if(RandomScoutRarity >= 24){ // 80, 21
    ScoutRarity = "R";
  }
  else if(RandomScoutRarity >= 9 && RandomScoutRarity <= 23){ // 81 - 95, 6 - 20
    ScoutRarity = "SR";
  }
  else if(RandomScoutRarity >= 5 && RandomScoutRarity <= 8){ // 96 - 99, 2 - 5
    ScoutRarity = "SSR";
  }
  else if(RandomScoutRarity <= 4){
    ScoutRarity = "UR";
  }
