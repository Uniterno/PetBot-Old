...
module.exports = class SkipGuessCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'sw',
            group: 'util',
            ...
            memberName: 'sw',
            aliases: ['skipwaiting'],
            description: 'Guessing event (Skip Waiting).',
            throttling: {
        usages: 3,
        duration: 2
    },
  ...
  if(!!GuessedNumber == false){ // Returns false for null,undefined,0,000,"",false.
    ...
    if(LastPlayer != UserID){
      message.channel.send("WARNING! You can use !guess without waiting. Execution of this command has been aborted.");
      return;
    }

    let MakiCookies = fs.readFileSync("./PetBot/settings/PetCoins/"+UserID+"/MakiCookies.txt");
    if(MakiCookies > 0){
      MakiCookies--;
      message.channel.send("You have paid 1 MakiCookie to skip the waiting.");
     fs.writeFileSync("./PetBot/settings/PetCoins/"+UserID+"/MakiCookies.txt",MakiCookies);
    } else{
