...
module.exports = class ItemGacha extends Command {
  ...
  if(CurrentDate != fs.readFileSync("./PetBot/settings/Christmas/TimesPulled/Date_"+UserID+".txt")){
    fs.writeFileSync("./PetBot/settings/Christmas/TimesPulled/Date_"+UserID+".txt",CurrentDate);
    fs.writeFileSync("./PetBot/settings/Christmas/TimesPulled/Times_"+UserID+".txt", 0);
  }

  let TimesPulledToday = parseInt(fs.readFileSync("./PetBot/settings/Christmas/TimesPulled/Times_"+UserID+".txt"));

  var Data = JSON.parse(fs.readFileSync("./PetBot/settings/Christmas/AvailableItems_" + UserID+ ".json", 'utf8'));

  let Total = Data.PetCoins100 + Data.PetCoins500 + Data.PetCoins1000 + Data.BT + Data.SRTicket + Data.SSRTicket + Data.URTicket + Data.MakiCookies5 + Data.MakiCookies25 + Data.DailyStreakFixer10 + Data.Badge + Data.Aureus10 + Data.PetCookies1 + Data.PetStars5 + Data.DivineItemExchangeTicket + Data.PadoruStickers + Data.ChristmasGifts + Data.ChristmasBells;

  if(Total <= 0){
    message.channel.send("Current pool has no remaining items. How did you manage to do this? Padoru is not amused.");
    return;
  }
  ...
  let Amount = 0;
    ...
    let CurrentPull = MasterUser[UserID].ChristmasPull;
    CurrentPull = parseInt(CurrentPull);
  ...
  if(Item <= Data.Badge){
    ...
    Got = "PetStars";
    Amount = 5;
  } else if(Item <= Data.Badge + Data.DailyStreakFixer10 + Data.MakiCookies25 + Data.MakiCookies5 + Data.URTicket + Data.SSRTicket + Data.SRTicket + Data.BT + Data.PetCoins1000 + Data.PetCoins500 + Data.PetCoins100 + Data.Aureus10 + Data.PetCookies1 + Data.PetStars5 + Data.DivineItemExchangeTicket){
    message.channel.send("["+message.author.username+"] (Cost: [Current] "+Cost+" [Next] "+CostNext+" PetTickets) You obtained **1 Divine Item Exchange Ticket**");
    Data.DivineItemExchangeTicket = Data.DivineItemExchangeTicket - 1;
    Got = "DivineItemExchangeTicket";
    Amount = 10;
  } else if(Item <= Data.Badge + Data.DailyStreakFixer10 + Data.MakiCookies25 + Data.MakiCookies5 + Data.URTicket + Data.SSRTicket + Data.SRTicket + Data.BT + Data.PetCoins1000 + Data.PetCoins500 + Data.PetCoins100 + Data.Aureus10 + Data.PetCookies1 + Data.PetStars5 + Data.DivineItemExchangeTicket + Data.PadoruStickers) {
    message.channel.send("["+message.author.username+"] (Cost: [Current] "+Cost+" [Next] "+CostNext+" PetTickets) You obtained **1 Padoru Sticker**");
    Data.PadoruStickers = Data.PadoruStickers - 1;
    Got = "PadoruStickers";
    Amount = 1;
  } else if(Item <= Data.Badge + Data.DailyStreakFixer10 + Data.MakiCookies25 + Data.MakiCookies5 + Data.URTicket + Data.SSRTicket + Data.SRTicket + Data.BT + Data.PetCoins1000 + Data.PetCoins500 + Data.PetCoins100 + Data.Aureus10 + Data.PetCookies1 + Data.PetStars5 + Data.DivineItemExchangeTicket + Data.PadoruStickers + Data.ChristmasGifts){
    message.channel.send("["+message.author.username+"] (Cost: [Current] "+Cost+" [Next] "+CostNext+" PetTickets) You obtained **1 Christmas Gift**");
    Data.ChristmasGifts += -1;
    Got = "ChristmasGifts";
    Amount = 50;
  } else{
    message.channel.send("["+message.author.username+"] (Cost: [Current] "+Cost+" [Next] "+CostNext+" PetTickets) You obtained **1 Christmas Bell**");
    Data.ChristmasBells += -1;
    Got = "ChristmasBells";
    Amount = 5;
  }
  ...
  if(Got == "PetCoins" && EquippedDivineItem == "Princess of Halloween's Pumpkin"){
    ...
    if(Got != "Nothing" && Got != "DivineItemExchangeTicket" && Got != "StreakFixer" && Got != "ChristmasBells" && Got != "ChristmasGifts" && Got != "PadoruStickers"){
      CurrentPull = fs.readFileSync("./PetBot/settings/PetCoins/"+UserID+"/"+Got+".txt");
      CurrentPull = parseInt(CurrentPull);
      fs.writeFileSync("./PetBot/settings/PetCoins/"+UserID+"/"+Got+".txt", CurrentPull+Amount);
    }

    if(Got == "StreakFixer"){
       CurrentPull = fs.readFileSync("./PetBot/settings/PetCoins/"+UserID+"/StreakFixers.txt");
       fs.writeFileSync("./PetBot/settings/PetCoins/"+UserID+"/StreakFixers.txt", CurrentPull + ", " + Amount + "%");
    }


    fs.writeFileSync("./PetBot/settings/Christmas/TimesPulled/Times_"+UserID+".txt", parseInt(TimesPulledToday) + 1);

    let FixedJSON = JSON.stringify(Data, null, "\t");
    fs.writeFileSync("./PetBot/settings/Christmas/AvailableItems_" + UserID + ".json",FixedJSON);
 }
}
