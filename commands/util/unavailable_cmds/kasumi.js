...
module.exports = class KasumiSkillCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'kasumi',
            group: 'util',
            memberName: 'kasumi',
            description: 'Kalmen command (Kasumi).',
            throttling: {
        usages: 5,
        duration: 10
    },
  ...
  if(Type != "10+1"){
    ...
    if(fs.existsSync("./PetBot/settings/skills/KasumiSkill.txt") == false){
      fs.writeFileSync("./PetBot/settings/skills/KasumiSkill.txt","0");
      }      
      let KasumiSkill = fs.readFileSync("./PetBot/settings/skills/KasumiSkill.txt");

      if(KasumiSkill >= 10){
        console.log("Can't use Kasumi skill anymore");
        return;
      }
...
/*if(ScoutRarity == "UR"){
    if(SpecialChance <= 50){
      request_url = request_url + "&ids=1582";
      DisableEvent = true; 
    }
  } else if(ScoutRarity == "SSR"){
    if(SpecialChance <= 50){
      request_url = request_url + "&ids=1581";
      DisableEvent = true;
    }
  }  
  if(ScoutRarity == "SR"){
    if(SpecialChance <= 50){
        request_url = request_url + "&ids=1579,1580,1583,1584,1585";
        DisableEvent = true;
    }
...
function PetCoinsSoldMessage(Amount, message, LargeScout, AmountToRemove){
   let UserID = message.author.id;
    message.channel.send("You got "+Amount+" (+"+parseInt(Amount/10)+" bonus) PetCoins as a result of the scout.");
    Amount = Amount * 1.1;
    ...
    NewValue = parseInt(NewValue) + parseInt(Amount);
     ...
     let CurrentTimesUsedSkill = fs.readFileSync("./PetBot/settings/skills/KasumiSkill.txt");
      fs.writeFileSync("./PetBot/settings/skills/KasumiSkill.txt",parseInt(CurrentTimesUsedSkill) + 1);
