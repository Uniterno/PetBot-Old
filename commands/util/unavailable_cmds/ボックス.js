...
module.exports = class ScoutCommand extends Command {
   ...
   async run(message, args) {
	...
	let Range = args[1];
 	...
 	let AmountToRemove; // This is the amount to remove of a certain Object.
      ...
      message.channel.send(ObjectToUpdate+"数が足りない。."); 			return;
 		}
  ...
  if(ScoutRarity == "UR"){
    if(SpecialChance <= 50){
      request_url = request_url + "&ids=1328,1411";
      DisableEvent = true;
    }
  } else if(ScoutRarity == "SSR"){
    if(SpecialChance <= 50){
      request_url = request_url + "&ids=1368";
      DisableEvent = true;
    }
  } else if(ScoutRarity == "SR"){
    if(SpecialChance <= 50){
      request_url = request_url + "&ids=1442";
      DisableEvent = true;
    }
  }
  ...
  res.on('end', () => {
    try {
      ...
      fixPageSize(num,LargeScout,Type,0,message, SpecialChance, EventChance); // previous 'getScoutingWithCorrectPageSize', to avoid null request due to data being stored in pages
    } catch (e) {
...
function fixPageSize(num,LargeScout,NumberOfScouting,Type, message, SpecialChance, EventChance){
...
if(ScoutRarity == "UR"){
  ...
  res.on('end', () => {
    try {
      ...
      let num = Math.floor(getRandom(0, parsedData.count));
      let LargeScout = true;
     fixPageSize(num,LargeScout,NumberOfScouting,Type, message, SpecialChance, EventChance);
    } catch (e) {
      console.error(e.message);
    }
  });
