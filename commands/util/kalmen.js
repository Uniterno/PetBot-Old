...
var UsedTimesSkill = parseInt(fs.readFileSync("./PetBot/settings/skills/DiaSkill.txt"));
...
module.exports = class DiaSkillCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'kalmen',
            group: 'util',
   ...
   async run(message, args) {
    UsedTimesSkill = parseInt(fs.readFileSync("./PetBot/settings/skills/DiaSkill.txt"));


      let UserID = message.author.id;

      if(UserID != "257191130855243777" && UserID != "168389193603612673"){
        message.channel.send("You require Dia skill in order to use this command.");
        return;
      }
  ...
  Type = "10+1";
    ...
    if(fs.existsSync("./PetBot/settings/skills/DiaSkill.txt") == false){
        fs.writeFileSync("./PetBot/settings/skills/DiaSkill.txt","1");
    }


    if(UsedTimesSkill > 12){
      message.channel.send("You have already used this command 12 times.");
      return;
    }

    if(UsedTimesSkill <= 9){
      AmountToRemove = AmountToRemove*0.9;
    }
   ...
   let ObjectToUpdateString = GetObjectToUpdateString(ObjectToUpdate, AmountToRemove);
      ...
      if(AmountToRemove == 1){
        message.channel.send(AmountToRemove+" "+ObjectToUpdate+" has been removed from your inventory to perform this action");
      }  else{
...
function fixPageSize(num,LargeScout,NumberOfScouting,Type, message, EventChance){
  ...
  res.on('end', () => {
    try {
      ...
      if(ScoutRarity == "SR"){
            SoldPetCoins = SoldPetCoins+30;
            console.log("30 Sold. Total: "+SoldPetCoins);
          } else if(ScoutRarity == "SSR"){
            SoldPetCoins = SoldPetCoins+220;
            console.log("220 Sold. Total: "+SoldPetCoins);
          } else if(ScoutRarity == "UR"){
            SoldPetCoins = SoldPetCoins+1900;
            console.log("1900 Sold. Total: "+SoldPetCoins);
          }
      	...
      	if(parsedData.results[Math.floor(num%10)].card_image){
            ...
            ScoutingResult[NumberOfScouting] = "https:"+parsedData.results[Math.floor(num%10)].round_card_image;
             ...
             fs.writeFileSync("./PetBot/settings/scout/Running.txt", 'false');
              setTimeout(PetCoinsSoldMessage, 4000, SoldPetCoins, message, LargeScout);
...
function getRandom(min, max) {
  ...
  function PetCoinsSoldMessage(Amount, message, LargeScout){
   let UserID = message.author.id;
    ...
    if(Amount < 300){
      if(UsedTimesSkill <= 9){
         message.channel.send("You have received your spent PetCoins (450) back since your scout wasn't worth 300 PetCoins.");
         NewValue = NewValue + 450;
      } else{
        message.channel.send("You have received your spent PetCoins (500) back since your scout wasn't worth 300 PetCoins.");
    ...
    fs.writeFileSync("./PetBot/settings/skills/DiaSkill.txt",UsedTimesSkill);
