...
module.exports = class BandoriCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'sif_aprilfools2020deprecated',
            group: 'util',
            guildOnly: true,
            memberName: 'sif_aprilfools2020deprecated',
  ...
  let CardsArray = JSON.parse(fs.readFileSync("./PetBot/settings/bandori/cardsSIFAprilFools.json"));
  let CardsArrayTemp = [];
  var ImportantArray = [];
  var ScoutingResult = [];
 ...
 if(Rarity == 4 && RateUp4 <= 800){
  ...
  let IdolizedRate = getRandom(0, 100);


  if(IdolizedRate <= 50){
      Idolized = "s";
  } else{
    Idolized = "AprilFools_notIdolized";
  }
